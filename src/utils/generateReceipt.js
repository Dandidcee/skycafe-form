...
/**
 * Generate receipt from Supabase order row (untuk Cek Status)
 * Converts DB row format to receipt HTML with current status
 */
export function generateReceiptFromOrder(order) {
  const statusLabels = {
  ...
  let bookingDetail = ''
  if (order.tipe === 'tempat' && order.tempat_nama) {
    bookingDetail = `
      <div class="section">
        <div class="section-title">Detail Booking Tempat</div>
        <div class="row"><span>Lokasi</span><span>${order.tempat_nama}</span></div>
        <div class="row"><span>Tanggal</span><span>${formatTanggal(order.tempat_tanggal)}</span></div>
        <div class="row"><span>Jam Mulai</span><span>${order.tempat_waktu || '-'}</span></div>
        <div class="row"><span>Durasi</span><span>${order.tempat_durasi} Jam</span></div>
        <div class="row"><span>Jumlah Orang</span><span>${order.tempat_orang}</span></div>
        <div class="divider"></div>
        <div class="row bold"><span>Subtotal Tempat</span><span>${formatRupiah(order.tempat_subtotal)}</span></div>
      </div>
    `
  }

  if (order.tipe === 'villa' && order.villa_checkin) {
    bookingDetail = `
      <div class="section">
        <div class="section-title">Detail Booking Villa</div>
        <div class="row"><span>Check-in</span><span>${formatTanggal(order.villa_checkin)}</span></div>
        <div class="row"><span>Check-out</span><span>${formatTanggal(order.villa_checkout)}</span></div>
        <div class="row"><span>Jumlah Malam</span><span>${order.villa_malam} malam</span></div>
        <div class="row"><span>Jumlah Tamu</span><span>${order.villa_tamu} orang</span></div>
        <div class="divider"></div>
        <div class="row bold"><span>Subtotal Villa</span><span>${formatRupiah(order.villa_subtotal)}</span></div>
      </div>
    `
  }
  ...
  if (order.menu_items && order.menu_items.length > 0) {
    const items = order.menu_items.map(i => `
      <div class="row">
        <span>${i.nama} <span class="qty">×${i.qty}</span></span>
        <span>${formatRupiah(i.subtotal)}</span>
      </div>
    ...
    menuDetail = `
      <div class="section">
        ...
        ${items}
        <div class="divider"></div>
        <div class="row bold"><span>Subtotal Menu</span><span>${formatRupiah(order.menu_subtotal)}</span></div>
      </div>
    `
  }

  const currentStatus = statusLabels[order.status] || 'Menunggu Dikonfirmasi'
  const statusColor = statusColors[order.status] || '#EAB308'
  ...
  <div class="receipt">
    ...
    <div class="body">
      <div class="meta">
        ...
        <div class="meta-item">
          <span class="meta-label">Status</span>
          <span class="meta-value" style="color:${statusColor}">${currentStatus}</span>
        </div>
      </div>

      <div class="section">
        <div class="section-title">Data Pemesan</div>
        <div class="row"><span>Nama</span><span>${order.nama}</span></div>
        <div class="row"><span>WhatsApp</span><span>${order.telepon}</span></div>
        ${order.email ? `<div class="row"><span>Email</span><span>${order.email}</span></div>` : ''}
      </div>
      ...
      <div class="section">
        <div class="section-title">Catatan</div>
        <p style="font-size:13px;color:#555">${order.catatan}</p>
      </div>
      ` : ''}

      <div class="total-section">
        <div class="total-row"><span>Subtotal</span><span>${formatRupiah(order.subtotal)}</span></div>
        <div class="total-row"><span>Biaya Layanan</span><span>${formatRupiah(order.biaya_layanan)}</span></div>
        <div class="total-row main"><span>Total</span><span>${formatRupiah(order.total)}</span></div>
      </div>
    </div>
